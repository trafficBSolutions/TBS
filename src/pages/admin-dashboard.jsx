...
<div className="tools-grid">
  ...
  <>
    ...
    <div style={{display:'flex',gap:'0.5rem',justifyContent:'center',marginBottom:'1.5rem'}}>
      <button onClick={async () => { setClockLocation('North GA'); axios.get('/timeclock/employees?location=North%20GA').then(r => { setPinEmployees(r.data.employees); }).catch(() => {}); axios.get('/timeclock/status').then(r => setClockedInList(r.data)).catch(() => {}); const sat = new Date(); sat.setDate(sat.getDate() - ((sat.getDay() + 1) % 7)); const satStr = `${sat.getFullYear()}-${String(sat.getMonth()+1).padStart(2,'0')}-${String(sat.getDate()).padStart(2,'0')}`; const fri = new Date(sat); fri.setDate(sat.getDate() + 6); const friStr = `${fri.getFullYear()}-${String(fri.getMonth()+1).padStart(2,'0')}-${String(fri.getDate()).padStart(2,'0')}`; setTimeWorkedWeekStart(satStr); try { const res = await axios.get(`/timeclock/time-worked?location=North%20GA&startDate=${satStr}&endDate=${friStr}`); setTimeWorked(res.data); } catch(e) {} }} style={{padding:'0.6rem 1.5rem',borderRadius:'8px',border:'2px solid',borderColor: clockLocation === 'North GA' ? '#1565c0' : '#ccc',background: clockLocation === 'North GA' ? '#1565c0' : '#fff',color: clockLocation === 'North GA' ? '#fff' : '#333',fontWeight:'bold',cursor:'pointer',fontSize:'1rem'}}>📍 North GA Time Clock</button>
      <button onClick={async () => { setClockLocation('South GA'); axios.get('/timeclock/employees?location=South%20GA').then(r => { setPinEmployees(r.data.employees); }).catch(() => {}); axios.get('/timeclock/status').then(r => setClockedInList(r.data)).catch(() => {}); const sat = new Date(); sat.setDate(sat.getDate() - ((sat.getDay() + 1) % 7)); const satStr = `${sat.getFullYear()}-${String(sat.getMonth()+1).padStart(2,'0')}-${String(sat.getDate()).padStart(2,'0')}`; const fri = new Date(sat); fri.setDate(sat.getDate() + 6); const friStr = `${fri.getFullYear()}-${String(fri.getMonth()+1).padStart(2,'0')}-${String(fri.getDate()).padStart(2,'0')}`; setTimeWorkedWeekStart(satStr); try { const res = await axios.get(`/timeclock/time-worked?location=South%20GA&startDate=${satStr}&endDate=${friStr}`); setTimeWorked(res.data); } catch(e) {} }} style={{padding:'0.6rem 1.5rem',borderRadius:'8px',border:'2px solid',borderColor: clockLocation === 'South GA' ? '#e65100' : '#ccc',background: clockLocation === 'South GA' ? '#e65100' : '#fff',color: clockLocation === 'South GA' ? '#fff' : '#333',fontWeight:'bold',cursor:'pointer',fontSize:'1rem'}}>📍 South GA Time Clock</button>
    </div>

    {/* Currently Clocked In */}
    <div style={{background:'#f0fdf4',border:'1px solid #86efac',borderRadius:'10px',padding:'1rem',marginBottom:'1.5rem'}}>
      <h4 style={{color:'#166534',margin:'0 0 0.75rem'}}>🟢 Currently Clocked In — {clockLocation} ({clockedInList.filter(entry => pinEmployees.some(emp => emp._id === entry.employeeId)).length})</h4>
      {clockedInList.filter(entry => pinEmployees.some(emp => emp._id === entry.employeeId)).length === 0 && <p style={{color:'#666',margin:0}}>No {clockLocation} employees currently clocked in.</p>}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(250px, 1fr))',gap:'0.75rem'}}>
        {clockedInList.filter(entry => pinEmployees.some(emp => emp._id === entry.employeeId)).map((entry) => (
                          ...
                          {dayData && dayData.records ? dayData.records.map((r, idx) => (
                            <div key={idx} style={{marginBottom: idx < dayData.records.length - 1 ? '6px' : 0}}>
                              ...
                              {r.purpose && <span style={{display:'block',marginTop:'2px',background:'#e3f2fd',color:'#1565c0',padding:'1px 6px',borderRadius:'3px',fontSize:'0.7rem',maxWidth:'fit-content',margin:'2px auto 0'}}>{r.purpose}{canEditHours && <button style={{marginLeft:'4px',padding:'0 3px',fontSize:'9px',background:'#1565c0',color:'#fff',border:'none',borderRadius:'2px',cursor:'pointer'}} onClick={async () => {
                                const purposes = ['2 Man Crew','3 Man Crew','4 Man Crew','Arrow Board/Message Board Job','Emergency Job','Weekend Work','Shop Work','Standby'];
